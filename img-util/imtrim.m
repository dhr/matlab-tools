function [out1,out2,out3,out4,out5] = imtrim(im,location)
  %IMTRIM Auto-crop an image like Photoshop's Edit>Trim feature.
  %   CROPPED = IMTRIM(IM) crop image based on top left corner. As of now
  %   only grayscale imagse are supported.
  %
  %   [cropped,t,b,l,r] = IMTRIM(IM) return cropped image and indices used
  %   to crop the image. So cropped = im(t:b,l:r);
  %
  %   [t,b,l,r] = IMTRIM(IM) return only indices used to crop the image. So
  %   cropped = im(t:b,l:r);
  %
  %   [...] = IMTRIM(IM,location) same as above but location may specify
  %   top-left corner ('NorthWest') or bottom-right corner ('SouthEast') to
  %   be the picel used in determining the auto-crop
  %
